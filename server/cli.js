 ...
 * Claude Web UI CLI
 *
 * Provides command-line utilities for managing Claude Web UI
 *
 * Commands:
...
// Match the runtime fallback in load-env.js so "claude-web-ui status" reports the same default
...
function showStatus() {
    ...
    console.log('\n' + c.dim('═'.repeat(60)));
    console.log(`\n${c.tip('[TIP]')} Hints:`);
    console.log(`      ${c.dim('>')} Use ${c.bright('claude-web-ui --port 8080')} to run on a custom port`);
    console.log(`      ${c.dim('>')} Use ${c.bright('claude-web-ui --database-path /path/to/db')} for custom database`);
    console.log(`      ${c.dim('>')} Run ${c.bright('claude-web-ui help')} for all options`);
...
function showHelp() {
    console.log(`
╔═══════════════════════════════════════════════════════════════╗
║           Claude Web UI - Command Line Tool             ║
╚═══════════════════════════════════════════════════════════════╝

Usage:
  claude-web-ui [command] [options]

Commands:
  start          Start the Claude Web UI server (default)
  sandbox        Manage Docker sandbox environments
  status         Show configuration and data locations
...
Report Issues:
  ${packageJson.bugs?.url || 'https://github.com/HeliosAiden/claude-web-ui/issues'}
`);
}
...
async function checkForUpdates(silent = false) {
    try {
        ...
        const latestVersion = execSync('npm show @heliosaiden/claude-web-ui version', { encoding: 'utf8' }).trim();
        const currentVersion = packageJson.version;

        if (isNewerVersion(latestVersion, currentVersion)) {
            console.log(`\n${c.warn('[UPDATE]')} New version available: ${c.bright(latestVersion)} (current: ${currentVersion})`);
            console.log(`         Run ${c.bright('claude-web-ui update')} to update\n`);
...
async function updatePackage() {
    try {
        ...
        execSync('npm update -g @heliosaiden/claude-web-ui', { stdio: 'inherit' });
        console.log(`${c.ok('[OK]')} Update complete! Restart claude-web-ui to use the new version.`);
    } catch (e) {
        console.error(`${c.error('[ERROR]')} Update failed: ${e.message}`);
        console.log(`${c.tip('[TIP]')} Try running manually: npm update -g @heliosaiden/claude-web-ui`);
    }
}
...
function showSandboxHelp() {
    console.log(`
${c.bright('Claude Web UI Sandbox')} — Run Claude Web UI inside Docker Sandboxes

Usage:
  claude-web-ui sandbox <workspace>            Create and start a sandbox
  claude-web-ui sandbox <subcommand> [name]    Manage sandboxes
...
Subcommands:
  ...
  ${c.bright('logs')}         Show Claude Web UI server logs
...
Examples:
  $ claude-web-ui sandbox ~/my-project
  $ claude-web-ui sandbox ~/my-project --agent codex --port 8080
  $ claude-web-ui sandbox ~/my-project --env SERVER_PORT=8080 --env HOST=0.0.0.0
  $ claude-web-ui sandbox ls
  $ claude-web-ui sandbox stop my-project
  $ claude-web-ui sandbox start my-project
  $ claude-web-ui sandbox rm my-project
...
async function sandboxCommand(args) {
    ...
    switch (opts.subcommand) {
        ...
        case 'stop':
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: claude-web-ui sandbox stop <name>\n`);
                process.exit(1);
            }
        ...
        case 'rm':
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: claude-web-ui sandbox rm <name>\n`);
                process.exit(1);
            }
        ...
        case 'logs':
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: claude-web-ui sandbox logs <name>\n`);
                process.exit(1);
            }
            try {
        ...
        case 'start': {
            if (!opts.name) {
                console.error(`\n${c.error('❌')} Sandbox name required: claude-web-ui sandbox start <name>\n`);
                process.exit(1);
            }
            ...
            console.log(`${c.info('▶')} Launching Claude Web UI server...`);
            sbx(['exec', opts.name, 'bash', '-c', 'claude-web-ui start --port 3001 &']);
            ...
            console.log(`\n${c.ok('✔')} ${c.bright('Claude Web UI is ready!')}`);
        ...
        case 'create': {
            if (!opts.workspace) {
                console.error(`\n${c.error('❌')} Workspace path required: claude-web-ui sandbox <path>\n`);
                console.log(`   Example: ${c.bright('claude-web-ui sandbox ~/my-project')}\n`);
                process.exit(1);
            }
            ...
            console.log(`\n${c.bright('Claude Web UI Sandbox')}`);
            ...
            // Step 3: Start Claude Web UI inside the sandbox
            console.log(`${c.info('▶')} Launching Claude Web UI server...`);
            sbx(['exec', opts.name, 'bash', '-c', 'claude-web-ui start --port 3001 &']);
            ...
            console.log(`\n${c.dim('  Or install globally:')} npm install -g @heliosaiden/claude-web-ui\n`);
            break;
        }
...
async function main() {
    ...
    switch (command) {
        ...
        default:
            ...
            console.log('   Run "claude-web-ui help" for usage information.\n');
            process.exit(1);
    }
}
