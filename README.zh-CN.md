...
<p align="center">
  <a href="https://discord.gg/buxwujPNRE">Discord</a> · <a href="https://github.com/HeliosAiden/claude-web-ui/issues">Bug 报告</a> · <a href="CONTRIBUTING.md">贡献指南</a>
</p>
...
- **插件系统** - 通过自定义选项卡、后端服务与集成扩展功能。 [开始构建 →](https://github.com/cloudcli-ai/cloudcli-plugin-starter)
...
启动自托管版本，只需一行 `npx`（需要 Node.js v22+）：
...
本项目是开源 UI 层。你可以在本地机器上自托管它，也可以使用提供团队功能与深入集成的云版本。
...
> 两种方式都使用你自己的 AI 订阅（Claude、Cursor 等）— 本工具提供环境，而非 AI。
...
本项目配备插件系统，允许你添加带自定义前端 UI 和可选 Node.js 后端的选项卡。在 Settings > Plugins 中直接从 Git 仓库安装插件，或自行开发。
...
自托管版本与云版本都是对 Claude Code 的扩展，而非旁观 — MCP 服务器、权限、设置、会话与 Claude Code 完全一致。

- **覆盖全部会话** — 自托管版本会自动扫描 `~/.claude` 文件夹中的每个会话。Remote Control 只暴露当前活动的会话。
- **设置统一** — 在 UI 中修改的 MCP、工具权限等设置会立即写入 Claude Code。
- **支持更多 Agents** — Claude Code、Cursor CLI、Codex、Gemini CLI。
- **完整 UI** — 除了聊天界面，还包括文件浏览器、Git 集成、MCP 管理和 Shell 终端。
- **云版本保持运行于云端** — 关闭本地设备也不会中断代理运行，无需监控终端。
...
需要。本工具只提供环境。你仍需自行获取 Claude、Cursor、Codex 或 Gemini 订阅。云版本从 $7/月起提供托管环境。

</details>

<details>
<summary>能在手机上使用吗？</summary>

可以。自托管时，在你的设备上运行服务器，然后在网络中的任意浏览器打开 `[yourip]:port`。云版本可从任意设备访问，内置原生应用也在开发中。
...
会的。自托管模式下，UI 会读取并写入 Claude Code 使用的 `~/.claude` 配置。通过 UI 添加的 MCP 服务器会立即在 Claude Code 中可见。
